...
# Configure logging
# Use container-compatible log path
...
if not os.path.exists(log_dir):
    try:
        ...
        log_dir = "."
        print("Warning: Could not create log directory, using current directory")

logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s - %(name)s - %(levelname)s - %(message)s",
    handlers=[
        logging.FileHandler(os.path.join(log_dir, "sre-app.log")),
...
APP_VERSION = Gauge(
    "app_version_info", "Application version", ["version", "build_date", "commit"]
)
...
def after_request(response):
    ...
    logger.info(
        ...
        f"{response.status_code} - {request_duration:.3f}s"
    )
