...
def test_application_performance(client):
    ...
    assert (
        response_time < 100
    ), f"Response time {response_time}ms exceeds 100ms threshold"
    assert response.status_code == 200
...
def test_concurrent_requests(client):
    ...
    # Make multiple requests sequentially to test request handling
    for i in range(10):
        response = client.get(f"/health?request={i}")
