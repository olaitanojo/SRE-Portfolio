...
class LogQuery(BaseModel):
    ...
    start_time: Optional[datetime] = Field(
        None, description="Start time for log search"
    )
...
async def search_logs(query: LogQuery, user=Depends(get_current_user)):
    """Search logs using Elasticsearch query"""
    try:
        # Build Elasticsearch query
        es_query = {
            "query": {"bool": {"must": [{"query_string": {"query": query.query}}]}},
        ...
        # Execute search
        response = await es_client.search(index=query.index_pattern, body=es_query)
        ...
        return {
            ...
            "query_info": {
                ...
                "query": query.query,
                "time_range": f"{query.start_time} to {query.end_time}"
                if query.start_time
                else "all time",
            },
        }
...
async def get_recent_logs(
    ...
    try:
        # Build query
        must_clauses = [{"range": {"@timestamp": {"gte": f"now-{minutes}m"}}}]
        ...
        return {
            "total": response["hits"]["total"]["value"],
            "results": results,
            "filters": {"minutes": minutes, "service": service, "log_level": log_level},
        }
...
async def analyze_logs(
    analysis_request: LogAnalysisRequest, user=Depends(get_current_user)
):
...
async def get_log_metrics(
    ...
    try:
        ...
        query = {
            "query": {"range": {"@timestamp": {"gte": es_time_range}}},
            "aggs": {
                "log_levels": {"terms": {"field": "log_level", "size": 10}},
                "services": {"terms": {"field": "service", "size": 20}},
                "errors_over_time": {
                    "filter": {"terms": {"log_level": ["ERROR", "FATAL", "CRITICAL"]}},
                    "aggs": {
                        "timeline": {
                            "date_histogram": {
                                "field": "@timestamp",
        ...
        return {
            ...
            "errors_timeline": response["aggregations"]["errors_over_time"]["timeline"][
                "buckets"
            ],
...
async def get_error_summary(
    ...
    try:
        query = {
            ...
            "aggs": {
                "error_groups": {
                    ...
                    "aggs": {
                        "error_types": {"terms": {"field": "log_level", "size": 10}},
                        "timeline": {
                            "date_histogram": {
                                "field": "@timestamp",
                                "calendar_interval": "1h",
                ...
                "top_errors": {"terms": {"field": "message.keyword", "size": 10}},
            },
            "size": 0,
        }
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get error summary: {str(e)}"
        )
...
async def perform_error_analysis(request: LogAnalysisRequest):
    ...
    query = {
        ...
        "aggs": {
            "error_patterns": {"terms": {"field": "message.keyword", "size": 20}},
            "services_affected": {"terms": {"field": "service", "size": 10}},
            "error_timeline": {
                "date_histogram": {"field": "@timestamp", "calendar_interval": "1h"}
            },
        },
    ...
    if request.services:
        query["query"]["bool"]["must"].append({"terms": {"service": request.services}})
...
async def perform_performance_analysis(request: LogAnalysisRequest):
    ...
    query = {
        ...
        "aggs": {
            "response_time_stats": {"stats": {"field": "response_time"}},
            "slow_requests": {
                "filter": {"range": {"response_time": {"gte": 5000}}},
                "aggs": {"by_service": {"terms": {"field": "service", "size": 10}}},
            },
...
async def perform_security_analysis(request: LogAnalysisRequest):
    ...
    query = {
        ...
        "aggs": {
            "security_events": {"terms": {"field": "event_type", "size": 20}},
            "failed_logins": {
                "filter": {
                    "bool": {
                        "must": [
                ...
                "aggs": {
                    "by_user": {"terms": {"field": "username", "size": 10}},
                    "by_ip": {"terms": {"field": "client_ip_hash", "size": 10}},
                },
            },
        },
        "size": 0,
