                        ...
                        for instance_id in instance_ids:
                            ...
                            for instance_datapoint in instance_response["Datapoints"]:
                                point = (
                                    ...
                                    .tag(
                                        "instance_type",
                                        instance_metadata[instance_id]["instance_type"],
                                    )
                                    .tag("metric", metric_config["metric"].lower())
                                    .field(
                                        "value",
                                        instance_datapoint[metric_config["stat"]],
                                    )
                                    .time(instance_datapoint["Timestamp"])
                                )

                                # Add tags from instance metadata
                                for tag_key, tag_value in instance_metadata[
                                    instance_id
                                ]["tags"].items():
                                    point = point.tag(
                                        f"tag_{tag_key.lower()}", tag_value
                                    )

                                self.write_api.write(bucket="metrics", record=point)

                logger.info(
                    f"Collected EC2 metrics for {len(instance_ids)} instances in {region}"
                )
    ...
    async def collect_lambda_metrics(self):
        ...
        for region, cloudwatch in self.cloudwatch_clients.items():
            try:
                ...
                logger.error(
                    f"Failed to collect Lambda metrics for {region}", exc_info=e
                )
    ...
    async def collect_cost_data(self):
        ...
        try:
            ...
            rightsizing_response = (
                self.cost_explorer_client.get_rightsizing_recommendation(
                    Service="AmazonEC2", PageSize=100
                )
            )

            for recommendation in rightsizing_response.get(
                ...
                if "RightsizingType" in recommendation:
                    point = (
                        ...
                        .field(
                            ...
                            float(
                                recommendation.get("EstimatedMonthlySavings", {}).get(
                                    "Amount", 0
                                )
                            ),
                        )
                    ...
                    if "ModifyRecommendationDetail" in recommendation:
                        ...
                        if "TargetInstances" in modify_detail:
                            ...
                            point = point.tag(
                                "recommended_type", target["InstanceType"]
                            )
            ...
            try:
                ...
                for reservation in instances["Reservations"]:
                    for instance in reservation["Instances"]:
                        if instance["State"]["Name"] == "running":
                            instance_type = instance["InstanceType"]
                            instance_count_by_type[instance_type] = (
                                instance_count_by_type.get(instance_type, 0) + 1
                            )
