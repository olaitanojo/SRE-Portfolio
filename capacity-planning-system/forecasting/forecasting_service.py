...
class CapacityForecaster:
    ...
    async def generate_forecast(
        self, metric: str, resource_id: str, config: ForecastConfig
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(historical_data) < config.min_data_points:
                raise ValueError(
                    f"Insufficient data points: {len(historical_data)} < {config.min_data_points}"
                )
            ...
            accuracy_metrics = (
                self._calculate_accuracy(model, df) if len(df) > 200 else None
            )
    ...
    async def _get_historical_data(self, metric: str, resource_id: str) -> pd.DataFrame:
        ...
        logger.info(
            f"Retrieved {len(df)} historical data points",
            metric=metric,
            resource_id=resource_id,
        )
        return df
    ...
    def _prepare_prophet_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        prophet_df = pd.DataFrame({"ds": df["time"], "y": df["value"]})
    ...
    async def _get_or_train_model(
        self, metric: str, resource_id: str, df: pd.DataFrame, config: ForecastConfig
    ) -> Prophet:
    ...
    def _generate_prophet_forecast(
        self, model: Prophet, config: ForecastConfig
    ) -> pd.DataFrame:
        ...
        forecast_only = forecast[
            forecast["ds"] > forecast["ds"].max() - pd.Timedelta(days=days)
        ]

        return forecast_only

    def _extract_forecast_results(
        self, forecast_df: pd.DataFrame, config: ForecastConfig
    ) -> Dict[str, Any]:
        ...
        results = {
            ...
            "summary": {
                ...
                "trend": "increasing"
                if forecast_df["trend"].iloc[-1] > forecast_df["trend"].iloc[0]
                else "decreasing",
            },
        }
        ...
        for _, row in forecast_df.iterrows():
            ...
            if config.confidence_levels:
                for confidence in config.confidence_levels:
                    ci_lower = row["yhat"] - (row["yhat_upper"] - row["yhat"]) * (
                        1 - confidence
                    )
                    ci_upper = row["yhat"] + (row["yhat_upper"] - row["yhat"]) * (
                        1 - confidence
                    )
    ...
    async def _get_cached_model(self, model_key: str) -> Optional[Prophet]:
        ...
        try:
            ...
            if model_data:
                ...
                model_age = (
                    datetime.utcnow() - model.history["ds"].max().to_pydatetime()
                )
                if model_age < timedelta(hours=24):
                    return model
        except Exception as e:
            logger.warning(
                "Failed to get cached model", exc_info=e, model_key=model_key
            )
    ...
    async def _cache_forecast(
        self, metric: str, resource_id: str, forecast: Dict[str, Any]
    ):
...
class ForecastingAPI:
    ...
    async def forecast_resource_capacity(
        ...
        forecast = await self.forecaster.generate_forecast(
            metric=f"{resource_type}_{metric}", resource_id=resource_id, config=config
        )

        # Add capacity planning insights
        forecast["insights"] = await self._generate_capacity_insights(
            forecast, resource_type
        )

        return forecast

    async def bulk_forecast_generation(
        ...
        for resource in resources:
            try:
                ...
                forecasts.append(
                    {"resource_id": resource["id"], "error": str(e), "status": "failed"}
                )

        return forecasts

    async def _generate_capacity_insights(
        self, forecast: Dict[str, Any], resource_type: str
    ) -> Dict[str, Any]:
        ...
        insights["growth_analysis"] = {
            ...
            "seasonality_detected": len(
                [
                    p
                    for p in timeline
                    if abs(p["predicted_value"] - mean_predicted) > mean_predicted * 0.2
                ]
            )
            > len(timeline) * 0.3,
        }
...
async def update_all_forecasts(api: ForecastingAPI, config: Dict[str, Any]):
    ...
    for resource_config in forecast_configs:
        try:
            ...
            logger.error(
                "Failed to update forecast", exc_info=e, resource_config=resource_config
            )
